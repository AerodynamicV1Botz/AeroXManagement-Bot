...
async def lego(event):
    ...
    try:
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ **Aero ✘ Management~🇮🇳**"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @AerodynamicV1_Promotion")


__mod_name__ = "Lᴏɢᴏ​"

__help__ = """
**Aero ✘ Management~🇮🇳** can create some beautiful and attractive logo for your profile pics.
